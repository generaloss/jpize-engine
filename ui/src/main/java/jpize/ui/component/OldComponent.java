...
public abstract class OldComponent<C>{
    ...
    public boolean isTouchDown(){
        return Jpize.isTouchDown() && isHover();
    }
    
    public boolean isTouched(){
        return Jpize.isTouched() && isHover();
    }
    
    public boolean isTouchReleased(){
        return Jpize.isTouchReleased() && grab;
    }
    ...
    @SafeVarargs
    protected final void setAsParentFor(UIComponent<C>... components){
    ...
    public void alignItems(Gravity alignComponents){
        this.alignComponents = alignComponents;
    }

    public void alignSelf(Gravity align){
        this.align = align;
    }
    ...
    private boolean checkIsHover(){
        ...
        if(!Jpize.window().isInputFocus() || mouseX < 0 || mouseX >= Jpize.getWidth() || mouseY < 0 || mouseY >= Jpize.getHeight())
            return false;

        boolean hover = Jpize.input().isInBounds(x, y, width, height);
    ...
    private float getAlignOffsetX(Gravity align){
    ...
    private float getAlignOffsetY(Gravity align){
    ...
    private int alignSignX(Gravity align){
        return align == RIGHT
            || align == RIGHT_DOWN
            || align == RIGHT_UP
            ? -1 : 1;
    ...
    private int alignSignY(Gravity gravity){
        return gravity == UP
            || gravity == LEFT_UP
            || gravity == RIGHT_UP
            ? -1 : 1;
    ...
    public UIComponent<C> copy(){
        try{
            return (UIComponent<C>) super.clone();
