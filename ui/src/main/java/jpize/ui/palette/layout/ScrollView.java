...
public class ScrollView extends AbstractLayout{
    ...
    public void render(){
        ...
        if(scroll != 0 && (super.isHovered() || handle.isHovered()))
    ...
    private void setupHandle(){
        ...
        handle.background().color().set(0.35, 0.1, 0.9);
        handle.setCornerRadius(Constr.relw(0.5));
        handle.setClickable(true);
        handle.addPressCallback((view, btn) -> {
            _handleGrabbed = true;
            _handleGrabY = Jpize.getY() - view.cache().y;
        });
        handle.addReleaseCallback((view, btn) -> _handleGrabbed = false);
    }
