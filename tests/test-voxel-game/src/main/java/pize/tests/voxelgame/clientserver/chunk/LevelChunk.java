...
public class LevelChunk{
    ...
    public short getBlock(int lx, int y, int lz){
        if(isOutOfBounds(y))
            return Block.VOID_AIR.getDefaultState();
        ...
        // Возвращаем блок
        return section.getBlock(lx, getLocalPos(y), lz);
    }
    ...
    public boolean setBlock(int lx, int y, int lz, short blockState){
        ...
        final int airID = Block.AIR.ID;
        final int blockID = BlockState.getID(blockState);
        ...
        if(section != null){
            ...
            if(section.getBlocksNum() == 1 && isBlockAir){
        ...
        final int ly = getLocalPos(y);
        final int oldBlockID = BlockState.getID(section.getBlock(lx, ly, lz));
    ...
    public BlockProperties getBlockProps(int lx, int y, int lz){
        return BlockState.getProps(getBlock(lx, y, lz));
    }
    
    public int getBlockID(int lx, int y, int lz){
        return BlockState.getID(getBlock(lx, y, lz));
    }
