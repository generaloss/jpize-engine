...
public class FontDemo extends JpizeApplication{
    ...
    private final float contentScaleX = 1;
    private final float contentScaleY = 1;
    ...
    public FontDemo(){
        ...
        final ByteBuffer fontFileData = Resource.internal("fonts/OpenSans-Regular.ttf").readByteBuffer();
...
//        lineOffset += Jpize.mouse().getScroll();
    ...
    public ByteBuffer ioResourceToByteBuffer(String resource, int bufferSize) throws IOException{
        ...
        try(InputStream source = FontDemo.class.getClassLoader().getResourceAsStream(resource)){
            assert source != null;
