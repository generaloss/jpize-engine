package jpize.tests.minecraftosp.server.gen.pool;
...
import static jpize.tests.minecraftosp.main.chunk.ChunkUtils.getChunkPos;
import static jpize.tests.minecraftosp.main.chunk.ChunkUtils.getLocalCoord;
...
public class BlockPool{
    ...
    public void loadBlocksFor(ServerChunk chunk){
        ...
        for(ChunkBlock block: blocks)
            if(chunk.getBlock(block.lx, block.y, block.lz) == Blocks.AIR)
                chunk.setBlockData(block.lx, block.y, block.lz, block.blockData);
    ...
    public void setBlockData(int x, int y, int z, short blockData){
        ...
        if(chunk == null || chunk != generatingChunk)
            ...
            chunk.setBlockData(lx, y, lz, blockData);
    }
